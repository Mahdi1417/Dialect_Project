...
class Eval:
    def __init__(self, task, model_name="Q1.5B", prompt_lang="ar", preds_folder="./ft_preds", prompt_type="ins", peft_method="lora"):
        self.task = task
        ...
        self.preds_file_path = os.path.join(self.preds_folder, "_".join([self.model_name, self.task, self.prompt_lang, self.prompt_type, self.peft_method]))
        ...
        self.eval_func_map = {
            ...
            "evaluate_irab": self.evaluate_irab,
        }
    ...
    def read_congifs(self):
        ...
        file_name = "_".join([self.model_name, self.task, self.prompt_lang, self.prompt_type, self.peft_method])+".txt"
    ...
    def get_preds(self):
        preds_folder = "_".join([self.model_name, self.task, self.prompt_lang, self.prompt_type, self.peft_method])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--peft_method', dest='peft_method', default='lora', help='lora, qlora, adalora, adapters, prefix, ia3')
    args=parser.parse_args()

    # assert args.model in ["Q1.5B", "Q7B", "Q14B"], "Invalid model!"
    assert args.prompt_lang in ["en", "ar"], "Only 'en' and 'ar' languages supported!"

    e = Eval(args.task, args.model, args.prompt_lang, prompt_type=args.prompt_type, peft_method=args.peft_method)
    e.evaluate()