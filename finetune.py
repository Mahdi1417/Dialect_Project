...
def finetune(args, logger):
    ...
    dataset_helper = FT_Dataset(tokenizer.eos_token, split="train", logger=logger, prompt_type=args.prompt_type)
    ...
    # -------------------------------------------------
    # Replace SFTTrainer with plain HF Trainer
    # -------------------------------------------------
    ...
    try:
        sample = dataset[0]
        if isinstance(sample, dict) and "input_ids" not in sample and "text" in sample:
        ...
        logger(f"Tokenization step failed, using raw dataset. Error: {e}\n")
        train_dataset = dataset
    ...
    training_args = TrainingArguments(
        ...
        lr_scheduler_type="cosine",
        seed=seed,
        output_dir=f"./outputs/{args.model}_{args.task}_{args.prompt_lang}_{args.prompt_type}_{args.peft_method}",
        save_strategy="no",
        report_to="none",
    ...
    model_path = os.path.join(args.save_path, f"{args.model}_{args.task}_{args.prompt_lang}_{args.prompt_type}_{args.peft_method}")
    os.mkdir(model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--peft_method', dest='peft_method', default='lora', help='lora, qlora, adalora, adapters, prefix')
    ...
    assert args.peft_method in ["lora", "qlora", "adalora", "adapters", "prefix", "ia3"], "Invalid peft_method!"
    assert args.prompt_type in ["ins", "int", "rp"], "Invalid prompt_type!"

    logger = Logger(os.path.join("./ft_logs/", f"{args.model}_{args.task}_{args.prompt_lang}_{args.prompt_type}_{args.peft_method}.txt"))
    logger("CONFIGS:")
