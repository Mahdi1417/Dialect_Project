...
class FT_Inference:
    def __init__(self, task, model_name="Q1.5B", prompt_lang="ar", models_dir="/data/ahmed026/ds", logs_dir="./ft_logs", prompt_type="ins", peft_method="lora"):
        self.task = task
        ...
        self.preds_file_path = os.path.join("./ft_preds", "_".join([self.model_name, self.task, self.prompt_lang, self.prompt_type, self.peft_method]))
    ...
    def load_model(self):
        file_name = "_".join([self.model_name, self.task, self.prompt_lang, self.prompt_type, self.peft_method])
...
if __name__ == "__main__":
    ...
    g = FT_Inference(args.task, args.model, args.prompt_lang, prompt_type=args.prompt_type, peft_method=args.peft_method)
